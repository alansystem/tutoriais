...
class image_CRUD {
	...
	//Modificado
	protected $nsu_veiculo = null;
	protected $id_loja = null;
	...
	//Modificado
	function set_nsu_veiculo($nsu_veiculo)
	{
		$this->nsu_veiculo = $nsu_veiculo;

		return $this;
	}

	//Modificado
	function set_id_loja($id_loja)
	{
	...
	protected function _upload_file($upload_dir) {
		...
		if (!empty($upload_response)) {
			...
			{
				$ci->image_moo->load($path)->resize(600,600)->save($path,true);
			}
			/* ------------------------------------- */		
    ...
    protected function _insert_table($file_name, $relation_id = null)
    {
    	...
    	$insert = array($this->url_field => $file_name,'nsu_veiculo' => $this->nsu_veiculo,'loja' => $this->id_loja);
